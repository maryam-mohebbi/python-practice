...
class Stream:
    ...
    class __Stream_op:
        ...
        def sort(self, comperator_func=lambda n1, n2: -1 if n1 < n2 else 1):
            ...
            # create new data for next __Stream_op instance from current instance
            # data: self.__data
            new_data = sorted(self.__data, key=cmp_to_key(
                comperator_func))      # <-- compute new data here
...
if __name__ == '__main__':
    ...
    def alpha_comperator(n1, n2): return -1 if n1 < n2 else 1

    def len_alpha_comperator(n1, n2): return -1 if len(n1) < len(
        n2) else 1 if len(n1) > len(n2) else alpha_comperator(n1, n2)
    #
    if run_choice == 41:
