...
class C3_names:
    ...
    def solution(self): # -> Self: from Python 3.10
        ...
        name_length_unique = list(dict.fromkeys(self.name_lengths))
        name_length_freq_tuple_list = [ (i, self.name_lengths.count(i)) for i in name_length_unique ]
        name_length_freq_tuple_list.sort(key=lambda tup: (tup[1], tup[0]), reverse=True)
        self.freq = {
            "most_freq": [],
            "least_freq": []
        }
        for i in range(0, 3):
            if (i >= len(name_length_freq_tuple_list)):
                break
            name_length = name_length_freq_tuple_list[i][0]
            name_frequency = name_length_freq_tuple_list[i][1]
            self.freq["most_freq"].append((name_frequency, name_length, [name for name in self.names if len(name) == name_length] ))
        for i in range(-1, -4, -1):
            if (i < -len(name_length_freq_tuple_list)):
                break
            name_length = name_length_freq_tuple_list[i][0]
            name_frequency = name_length_freq_tuple_list[i][1]
            self.freq["least_freq"].append((name_frequency, name_length, [name for name in self.names if len(name) == name_length] ))
        # My code #
    ...
    def avg_name_length(self) -> float:
        ...
        """
        # My code #
        return round(sum(self.name_lengths) / len(self.name_lengths), 2)
        # My code #
