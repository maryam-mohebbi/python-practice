...
class Stream_test(unittest.TestCase):
    ...
    def test_filter_13(self):
        result = Stream(self.names).source() \
            ...
            .get()
        self.assertEqual(['Gill', 'Howe', 'Case', 'Lott',
                         'Hall', 'Pena', 'Witt', 'Soto'], result)
    ...
    def test_sort_4(self):
        ...
        result = Stream(self.names).source() \
            ...
            .sort() \
            .get()
        expected = ['Buckner', 'Gill', 'Gonzalez', 'Hardin',
                    'Howe', 'Marquez', 'Ray', 'Richardson']
    ...
    def test_sort_42(self):
        ...
        expected = [('Cox', 'Xoc', 3), ('Ray', 'Yar', 3), ('Brock', 'Kcorb', 5), ('Casey', 'Yesac', 5),
                    ('Crane', 'Enarc', 5), ('Gomez', 'Zemog',
                                            5), ('Petty', 'Yttep', 5), ('Vance', 'Ecnav', 5),
                    ('Bernard', 'Dranreb', 7), ('Buckner',
                                                'Renkcub', 7), ('Marquez', 'Zeuqram', 7),
                    ('Navarro', 'Orravan', 7), ('Nielsen',
                                                'Neslein', 7), ('Raymond', 'Dnomyar', 7),
...
def run_tests(test_class):
    ...
    _failed = [(str(_test).split()[0], _trace.split(',')[1])
               for _test, _trace in _result.failures]
    for _f in _failed:
        _succ.remove(_f[0])
    #
    for t in _succ:
