...
class C2_numbers:
    ...
    def __init__(self, numbers=None):
        ...
        # b) initialize with first three numbers: [4, 12, 3]
        self.b = self.numbers[0:3]      # <-- write expression here
        ...
        # f) initialize with number of odd numbers: 4
        self.f = len([i for i in self.numbers if i % 2 == 1])
        ...
        # j) ascending list of squared numbers with no duplicates: [1, 9, 16, 49, 64, 144, 289]
        self.j = [i**2 for i in sorted(list(dict.fromkeys(self.numbers)))]

        # k) initialize with "ODD_LIST", "EVEN_LIST" or "EMPTY_LIST" depending on numbers length
        self.k = "EMPTY_LIST" if len(self.numbers) == 0 else "ODD_LIST" if len(self.numbers) % 2 == 1 else "EVEN_LIST"
        # My code #
